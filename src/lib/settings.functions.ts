...
export const exportEstablishmentData = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    const { supabase, userId } = context;
    await assertRole(supabase, userId, data.establishment_id, "owner");
    const { supabaseAdmin: adminForExport } = await import("@/integrations/supabase/client.server");
    const { data: est } = await adminForExport.from("establishments").select("*").eq("id", data.establishment_id).single();
